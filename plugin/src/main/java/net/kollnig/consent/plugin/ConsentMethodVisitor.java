...
public class ConsentMethodVisitor extends MethodVisitor {
    ...
    private static final String GPC_INTERCEPTOR = "net/kollnig/consent/standards/GpcInterceptor";
    ...
    private void injectConsentCheck() {
        if (rule.action == ConsentTransformRules.Action.INJECT_GPC_HEADER) {
            injectGpcHeader();
            return;
        }
    ...
    /**
     * Inject GPC header into an HTTP request builder's build() method.
     *
     * Works for any builder that has a header(String, String) method:
     * OkHttp3, OkHttp2, Cronet, etc.
     *
     * Generated bytecode equivalent:
     *   if (GpcInterceptor.isEnabled()) {
     *       this.header("Sec-GPC", "1");  // or addHeader(), etc.
     *   }
     *   // ... original build() code follows
     */
    private void injectGpcHeader() {
        Label skipLabel = new Label();

        // --- Generate: GpcInterceptor.isEnabled() ---
        mv.visitMethodInsn(
                Opcodes.INVOKESTATIC,
                GPC_INTERCEPTOR,
                "isEnabled",
        ...
        // --- If GPC not enabled, skip header injection ---
        mv.visitJumpInsn(Opcodes.IFEQ, skipLabel);

        // --- Generate: this.header("Sec-GPC", "1") ---
        // Uses the rule's headerMethodName and headerMethodDesc so this works
        // for any HTTP client (OkHttp3, OkHttp2, Cronet, etc.)
        mv.visitVarInsn(Opcodes.ALOAD, 0); // this (the builder instance)
        mv.visitLdcInsn("Sec-GPC");
        mv.visitLdcInsn("1");
        mv.visitMethodInsn(
                Opcodes.INVOKEVIRTUAL,
                rule.className,
                rule.headerMethodName,
                rule.headerMethodDesc,
                false);
        mv.visitInsn(Opcodes.POP); // discard returned builder (it's `this`)

        // --- skipLabel: original build() code follows ---
